...
`--all` が指定された場合は主要役職（villager, seer, werewolf, knight）で順次実行:

```bash
uv run python scripts/capture_ui.py --role villager
uv run python scripts/capture_ui.py --role seer
uv run python scripts/capture_ui.py --role werewolf
uv run python scripts/capture_ui.py --role knight
```
