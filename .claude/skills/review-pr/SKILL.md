...
1. `.py` ファイルの変更あり → **バグ・ロジック** + **セキュリティ** + **スタイル・設計** + **ドキュメント**
...
**TeamCreate でレビューチームを作成し、該当する各レビュアーをチームメイトとして並列に起動すること。**

手順:

1. `TeamCreate` で `team_name: "pr-review"` のチームを作成する
2. `TaskCreate` で該当レビュアーごとにタスクを作成する
3. 各レビュアーを `Task` ツール（`team_name: "pr-review"`, `subagent_type: general-purpose`）でチームメイトとして起動する
4. 各チームメイトのプロンプトには以下を含める:
   - PR 番号と差分内容（または差分の該当部分）
   - そのレビュアーのチェック観点（下記参照）
   - 「発見した問題を 必須修正 / 推奨修正 / 将来の課題 に分類して報告せよ」という指示
5. 全チームメイトの結果が返ってきたら `TeamDelete` でチームを削除し、Step 5 に進む
