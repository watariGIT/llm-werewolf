---
name: review-pr
description: PR の差分を最大5種のレビュアー観点 + /simplify でレビューする。PR 作成後のレビューや、ユーザーが「/review-pr」「PR をレビューして」「コードレビューお願い」と言ったときに使う。PR 番号の指定がなければ現在のブランチの PR を自動検出する。
---

# Review PR

Pull Request のコードレビューを行うスキル。差分を取得し、変更内容に応じて最大5種のレビュアーを選定してレビューし、さらに `/simplify` スキルによる簡素化チェックを行う。
...
4. 全サブエージェントの結果が返ってきたら Step 4.5 に進む
...
### Step 4.5: /simplify による簡素化チェック

`/simplify` スキルを実行し、PR の変更コードに対して再利用・品質・効率性の観点でチェックする。

- `/simplify` は変更されたコードを分析し、コードの再利用・品質・効率性に関する問題があれば修正を行う
- `/simplify` が修正を行った場合、修正内容を記録し Step 5 の結果分類に統合する
- `/simplify` が問題なしと判断した場合は「✅ 問題なし」として記録する

### Step 5: 結果の分類

各レビュアーと `/simplify` の結果を3レベルに分類する:

1. **🔴 critical** — バグ・セキュリティ・ルール不整合。この PR で必ず修正すべき
2. **🟡 warning** — 品質改善。今回直せるなら直したい。`/simplify` が修正を行った場合は「/simplify により修正済み」と記載する
...
### 簡素化チェック（/simplify）
- ✅ 問題なし
（修正があった場合 → 🟡 warning: /simplify により修正済み — <修正内容>）
