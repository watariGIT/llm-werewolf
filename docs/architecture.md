...
| `load_prompt_config` | 環境変数から `PromptConfig` を生成するファクトリ関数。`MAX_RECENT_STATEMENTS` でプレイヤーAIの発言ログ件数上限を設定可能（デフォルト: 20）。`GM_MAX_RECENT_STATEMENTS` でGM-AIの発言ログ件数上限を設定可能（デフォルト: 30） |
